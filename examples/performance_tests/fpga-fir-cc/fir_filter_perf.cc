...
int main() {
  ...
  int total_left = NUM_SAMPLES/2; // Half due to complex samples

  std::vector<int> taps;
  for (i = 0; i < (NUM_TAPS+1)/2; i++) {
    taps.push_back(i << 15);
  }

  // Place on heap as stack does not have enough space
  int* input_buffer = new int[NUM_SAMPLES];
  ...
  // Place on heap as stack does not have enough space
  int* output_buffer = new int[NUM_SAMPLES];
  ...
  gr::zynq::fir_filter_cc_impl *filter = new gr::zynq::fir_filter_cc_impl(taps);
